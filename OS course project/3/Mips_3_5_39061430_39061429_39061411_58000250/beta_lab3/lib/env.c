...
load_icode(struct Env *e, u_char *binary, u_int size)
{
	...
	//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
		/* allocate and copy binary (text+data) */
	int i, r;
	struct Page *p = NULL;
	   unsigned int *index;
       u_char *binary_copy;
	...
	//struct a_out_hdr *header = (void *)binary;  //header为elf文件的头部

	i = 0;
	while (i<size) {
	  if ((r = page_alloc(&p)) < 0)
	    panic("Couldn't alloc icode memory\n Can't start up\n");
	//if (page_insert(e->env_pgdir, p, UTEXT + (size - i), PTE_U|PTE_W|PTE_A))
	    if (page_insert(e->env_pgdir, p, UTEXT + i,PTE_R))
	      panic("Failed to insert rw icode page  %d -> %d \n", page2pa(p), UTEXT +i);
	  bcopy(binary + (i), (void *)page2kva(p),  BY2PG);
	  i += BY2PG;
	}
	...
	//if (page_insert(e->env_pgdir, p, USTACKTOP-BY2PG, PTE_U|PTE_W|PTE_A))
	if (page_insert(e->env_pgdir, p, USTACKTOP-BY2PG, PTE_R))
	  panic("Failed to insert icode stack page\n");	

	index=binary;
	  e->env_tf.pc=index[6];
	printf("iload success\n");
...
env_create(u_char *binary, int size)
{
	  ...
	  //strncpy(e->env_name, name, ENV_NAME_LEN);
	  //e->env_name[ENV_NAME_LEN-1] = '\0';

	  load_icode(e, binary, size);
          printf("env_creat success");
