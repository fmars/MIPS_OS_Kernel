...
{
	return msyscall(SYS_set_trapframe,envid,tf,0,0,0);
}

void
syscall_panic(char *msg)
{
	msyscall(SYS_panic,msg,0,0,0,0);
}
...
int  
syscall_set_prio(u_int envid, u_int level)
{
	return 	msyscall(SYS_set_prio, envid , level, 0, 0, 0);
}
