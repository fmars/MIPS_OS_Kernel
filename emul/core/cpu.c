...
execute_inst_1(void)
{
	int bad_inst = 0;
        ...
        static int open=0; //pds--------------

        if (open==0)
        {
           fp_pds=fopen("info.txt","w");
	   open=1;
	}
	...
	fprintf(fp_pds,"Addr:0x%8x  Inst:0x%8x",cpu_pc-4,inst);  // pds
	if (bad_inst) // pds--------------------------------------------
        {	 
            fprintf(fp_pds," ------------------------------------ unknown instruction -------------------------------------------------");
	}
       fprintf(fp_pds,"\n");
        fflush(fp_pds);
...
execute_eretx(void)
{
	...
	cp0Reg.cause &= 0xFFFFFF83; // !!! pds-----------  cause ExcCode reset to 0 ---------------------

	eret_cp0_check_flag=1; // pds----------------
}
