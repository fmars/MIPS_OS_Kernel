...
/*		return;*/
	...
	// exccode_accept_way = 0x00001731u;  0001_0111_0011_0001
	/* old pds -----------------
	if ((exccode_accept_way & (1u << (exccode ? (exccode - 1) : 0))) == 0)
		return;
	*/

	if ((exccode_accept_way & (1u << exccode)) == 0) // exception for syscall and break  pds-----------------
		return;
	...
	/* disable anymore exceptions. */   //  ????  pds---------------
	...
	else if (exccode==CP0_CAUSE_EC_SC) // pds-----------
	{
		cp0Reg.cause |= (1<<8);
		//printf("This is syscall exception EPC=0x%x...\n",epc);  // pds---------------------
 	}
	else if (exccode==CP0_CAUSE_EC_BP) // pds ----------------------
		cp0Reg.cause |= (1<<9);
...
/* only do one time in one cycle. */
	...
	if (!exc_flag) {
		...
		if ((!exc_flag) && (cp0Reg.cause & 0x00000200u))  // This is breakpoint Exception
			exc_flag = 9;

		/* check software interrupt 0. */
		if ((!exc_flag) && (cp0Reg.cause & 0x00000100u))  // This is syscall Exception
			exc_flag = 8;
		...
		//cp0Reg.cause &= (~CP0_CAUSE_EC_MASK);  // by pds----------------------------
	...
	/* Get exccode */
	exccode = (cp0Reg.cause & CP0_CAUSE_EC_MASK) >> CP0_CAUSE_EC_SHIFT;  // pds ------- "<<" to ">>"
	...
	cp0Reg.sr &= ~CP0_SR_IE_MASK; // pds !!! Set IE t0 0 -------------------------

	if (cp0Reg.cause & 0x00000100) // clear syscall  pds------------------
	       cp0Reg.cause &=0xFFFFFEFFu;
      	else if (cp0Reg.cause & 0x00000200) // clear break
	       cp0Reg.cause &=0xFFFFFDFFu;
	else printf("Enter Interrupt or Exception Processing Program PC=0x%x........\n",cpu_pc);
